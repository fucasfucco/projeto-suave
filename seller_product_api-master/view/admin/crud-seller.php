...
<?php
$sellers = file_get_contents('http://localhost:3000/sellers');
$jsonObj = json_decode($sellers);

foreach ($jsonObj as &$seller){
   ...
   echo '<input type="hidden" name="ID" value="' . $seller->id . '">';
   echo  '<h5 class="card-title name">Nome: ' . $seller->name . '</h5>';
   echo '<hr><h5 class="card-title name">Comissão: ' . $seller->sellerComission . '</h5>';
   ...
   echo '<form action="http://localhost:3000/delete/seller/' . $seller->id .'" method="post"><button type="submit" onclick="reloads()" class="butao btn btn-danger">X</button></form>';
   echo '</div>';
   echo '</div>';
   echo '</div>';
    unset($seller);
